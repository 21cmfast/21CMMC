...
####################### START LikelihoodIsocBispecCoeval ##########################

class LikelihoodIsocBispecCoeval(LikelihoodBaseFile):
	"""
	A likelihood which assumes that the spherically-averaged isoceles bispectrum is iid Gaussian in each bin.
	...
	If a `datafile` is provided and the datafile exists, then the data will be read from that file. Otherwise, theoretical data will be automatically simulated to match current parameters. This will be written to `datafile` if provided.
	"""
	required_cores = [core.CoreCoevalModule]

	def __init__(self, min_k=0.1, max_k=1.0, obs_sim=False, norm=False, k1=0.1, model_uncertainty=0.15,
		...
		obs_sim : bool, optional !!! REMOVE THIS OPTION AND ADJUST C-CODE TO JUST MEAN SUBTRACT RATHER THAN MAKE DELTA
			Whether the uv samplng has been applied at some level (controls whether to mean subtract or not)
		norm : bool, optional
			Whether to normalise (True) the bispectrum according to Watkinson+ 2018 or not (False)
		k1 : float, optional (units 1/Mpc).
		logk : bool, optional (units 1/Mpc).
		...
		>>>        return {"theta_over_pi": data[:, 0], "k3": data[:, 1], "B": data[:, 2]}
	...
	def data_spline(self):
		...
		return [InterpolatedUnivariateSpline(d["theta_over_pi"], d["B"], k=1) for d in self.data]

	@cached_property
	def noise_spline(self):
		"""Splines of noise power spectra"""
		if self.noise:
			return [InterpolatedUnivariateSpline(n["theta_over_pi"], n["B_err"], k=1) for n in self.noise]
		else:
			return None

	@staticmethod
	def compute_bispec(brightness_temp, DIM, L, k1, OBS_SIM=False, NORM=False, ignore_kperp_zero=True, ignore_kpar_zero=False, ignore_k_zero=False):
		# Convert data to be complex
		dT_complex = brightness_temp.astype(np.complex)
		BS_isoc_dict = bifft.calculate_isoc_bipec(complex_data=dT_complex, DIM=[DIM], LEN=[L], k_1=k1, normalise=NORM)

		data = {"theta_over_pi": np.array(BS_isoc_dict["theta_over_pi"]), "k3": np.array(BS_isoc_dict["k3"]), "B": np.array(BS_isoc_dict["BS"])}
		#print("In bispec likelihood -> ", data)
	...
	def computeLikelihood(self, model):
		...
		for i, (m, pd) in enumerate(zip(model, self.data_spline)):
			...
			moduncert = (
				...
				else self.model_uncertainty * m["B"][mask]
			)
	...
	def reduce_data(self, ctx):
		...
		for bt in brightness_temp:
			...
			bispec_data.append( self.compute_bispec(brightness_temp=bt, DIM=self.user_params.HII_DIM, L=self.user_params.BOX_LEN, k1=self.k1, OBS_SIM=self.obs_sim, NORM=self.norm, ignore_kperp_zero=self.ignore_k_zero, ignore_kpar_zero=self.ignore_kpar_zero, ignore_k_zero=self.ignore_kperp_zero) )

			'''shpe = []
			shpe.append(bispec_data.BSbins)

			# cast to a new numpy array that we can delete to stop memory leak
			theta_over_pi = np.ctypeslib.as_array(bispec_data.BS_theta, shpe)
			k_3 = np.ctypeslib.as_array(bispec_data.BS_k3, shpe)
			bispec = np.ctypeslib.as_array(bispec_data.BS, shpe)

			data.append({"theta_over_pi": theta_over_pi, "k3": k_3, "B": bispec})

			del theta_over_pi, k_3, bispec

			return data'''
		return bispec_data

	def store(self, model, storage):
		# add the bispec to the written data
		for i, m in enumerate(model):
			storage.update({k + "_z%s" % self.redshift[i]: v for k, v in m.items()})

########################## END LikelihoodIsocBispecCoeval ##########################
